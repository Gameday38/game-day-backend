...
const environment = payPalMode === 'live' ? new paypal.core.LiveEnvironment(clientId, clientSecret) : new paypal.core.SandboxEnvironment(clientId, clientSecret);
...
app.post('/api/create-order', async function(req, res) {
  try {
    ...
    var isYearly = req.body.isYearly;
    var prices = { premium: { monthly: '9.99', yearly: '99.99' }, pro: { monthly: '19.99', yearly: '199.99' } };
    var price = prices[planType][isYearly ? 'yearly' : 'monthly'];
    var request = new paypal.orders.OrdersCreateRequest();
    request.requestBody({ intent: 'CAPTURE', purchase_units: [{ amount: { currency_code: 'USD', value: price } }] });
...
app.post('/api/capture-order', async function(req, res) {
  try {
